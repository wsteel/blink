...
bool WKE_CALL_TYPE onDownloadCallback(wkeWebView window, void *param, const char *url)
{
    goOnDownloadCallback(window, url);
    return true;
}
...
void initWebViewEvent(wkeWebView window)
{
    ...
    //download
    wkeOnDownload(window, onDownloadCallback, NULL);
}